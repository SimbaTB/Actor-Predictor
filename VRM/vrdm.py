...
class VRM(nn.Module):
    ...
    def forward_inference(self, prev_h_levels, prev_d_levels, x_obs, a_prev_obs):
        # 确保输入张量与隐藏状态在同一设备
        device = prev_d_levels[0].device
        x_obs = x_obs.to(device)
        a_prev_obs = a_prev_obs.to(device)
        ...
        if self.action_feedback:
            mu_levels = [self.f_daphi2mu_q[l](torch.cat((prev_d_levels[l], a_prev_obs, x_phi), dim=-1)) for l in
                        range(self.n_levels)]
            if isinstance(self.sig_scale, float):
                sig_levels = [torch.tensor(self.sig_scale, dtype=torch.float32, device=device) for l in range(self.n_levels)]
            else:
    ...
    def train_st(self, x_obs, a_obs, h_levels_0=None, d_levels_0=None, h_0_detach=True, validity=None, done_obs=None, seq_len=64):
        ...
        if validity is None:  # no need for padding
            validity = torch.ones([x_obs.size()[0], x_obs.size()[1]], requires_grad=False, device=device)

        if h_levels_0 is None:
            h_levels_0 = self.init_hidden_zeros(batch_size=batch_size, device=device)
        elif isinstance(h_levels_0[0], np.ndarray):
            h_levels_0 = [torch.from_numpy(h_0).to(device) for h_0 in h_levels_0]

        if d_levels_0 is None:
            d_levels_0 = self.init_hidden_zeros(batch_size=batch_size, device=device)
        elif isinstance(d_levels_0[0], np.ndarray):
            d_levels_0 = [torch.from_numpy(d_0).to(device) for d_0 in d_levels_0]
        ...
        stps_burnin = 64
        x_sampled = torch.zeros([x_obs.size()[0], seq_len, x_obs.size()[-1]], dtype=torch.float32, device=device)
        a_sampled = torch.zeros([a_obs.size()[0], seq_len, a_obs.size()[-1]], dtype=torch.float32, device=device)
        v_sampled = torch.zeros([validity.size()[0], seq_len], dtype=torch.float32, device=device)
    ...
    def init_hidden_zeros(self, batch_size=1, device=None):
        h_levels = [torch.zeros((batch_size, d_size), device=device) for d_size in self.d_layers]
        return h_levels


# 以下是修改后的 VRDM 类，主要改动集中在数据移动部分
class VRDM(nn.Module):
    def __init__(self,
        ...
        # 初始化隐藏状态（先设为 None，在使用时动态创建并移至设备）
        self.h_levels = None
        self.d_levels = None
        ...
        self.optimizer_e = torch.optim.Adam([self.log_beta_h], lr=lr_rl) if isinstance(self.beta_h, str) and self.beta_h.startswith('auto') else None

        self.mse_loss = nn.MSELoss()

    def _get_device(self):
        return next(self.fim.parameters()).device
    ...
    def preprocess_sac(self, x_obs, r_obs, a_obs, d_obs=None, v_obs=None, seq_len=64):
        ...
        x_sampled = torch.zeros([x_obs.size()[0], seq_len + 1, x_obs.size()[-1]], dtype=torch.float32, device=device)
        a_sampled = torch.zeros([a_obs.size()[0], seq_len + 1, a_obs.size()[-1]], dtype=torch.float32, device=device)
        ...
        for b in range(x_obs.size()[0]):
            ...
            if start_index >= 1:
                ...
                # [todo] for t_burnin in range(x_tmp.size()[1]):
        ...
        if d_obs is None:
            D = torch.zeros_like(R, dtype=torch.float32, device=device)
        else:
            D = d_obs

        if v_obs is None:
            V = torch.ones_like(R, requires_grad=False, dtype=torch.float32, device=device)
        else:
            V = v_obs

        A_sampled = torch.zeros([A.size()[0], seq_len + 1, A.size()[-1]], dtype=torch.float32, device=device)
        D_sampled = torch.zeros([D.size()[0], seq_len + 1, 1], dtype=torch.float32, device=device)
        R_sampled = torch.zeros([R.size()[0], seq_len + 1, 1], dtype=torch.float32, device=device)
        V_sampled = torch.zeros([V.size()[0], seq_len + 1, 1], dtype=torch.float32, device=device)
    ...
    def train_rl_sac_(self, S_sampled, SP_sampled, A_sampled, R_sampled, D_sampled, V_sampled,
        ...
        if computation == 'explicit':
            ...
            log_pi_exp = torch.sum(- (mua_tensor.data - sampled_u.data).pow(2)
                                     ...
                                     - torch.log(siga_tensor.data * torch.tensor(2.5066, device=siga_tensor.device)),
                                   dim=-1, keepdim=True)
            ...
            eps = (sampled_u.data - mua_tensor.data) / (siga_tensor.data)
        ...
        # ------ implicit computing---------------
            ...
            # [todo] sampled_u = mu_prob.rsample()
    ...
    def init_episode(self, x_0=None, h_levels_0=None, d_levels_0=None):
        device = self._get_device()
        if h_levels_0 is None:
            self.h_levels = self.init_hidden_zeros(batch_size=1, device=device)
        else:
            self.h_levels = [torch.from_numpy(h0).to(device) for h0 in h_levels_0]

        if d_levels_0 is None:
            self.d_levels = self.init_hidden_zeros(batch_size=1, device=device)
        else:
            self.d_levels = [torch.from_numpy(d0).to(device) for d0 in d_levels_0]

        if x_0 is None:
            x_obs_0 = None
        else:
            x_obs_0 = torch.from_numpy(x_0).view(1, -1).to(device)
    ...
    def select(self, s, r_prev, action_return='normal'):
        ...
        x_obs = torch.cat((torch.from_numpy(s).to(device), torch.from_numpy(r_prev).to(device))).view([1, -1])
    ...
    def learn_st(self, train_fim:bool, train_klm:bool, SP, A, R, D=None, V=None,
        ...
        # 输入是 numpy 数组，需要转为 tensor 并移到设备
        device = self._get_device()
        ...
        for xt in range(times):
            ...
            r_obs = torch.from_numpy(r.reshape([r.shape[0], r.shape[1], 1])).to(device)
            x_obs = torch.cat((torch.from_numpy(sp).to(device), r_obs), dim=-1)
            a_obs = torch.from_numpy(a).to(device)
            d_obs = torch.from_numpy(d.reshape([r.shape[0], r.shape[1], 1])).to(device)
            v_obs = torch.from_numpy(v.reshape([r.shape[0], r.shape[1], 1])).to(device)
